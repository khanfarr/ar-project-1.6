...
public class TORTimeAPI : MonoBehaviour
{
    ...
    private IEnumerator RefreshOnce()
    {
        ...
        using UnityWebRequest request = UnityWebRequest.Get("https://api.open-meteo.com/v1/forecast?latitude=43.6532&longitude=-79.3832&current_weather=true&timezone=America%2FToronto");
        request.timeout = 10;
        ...
        {
            Debug.LogWarning($"TORTimeAPI request failed: {request.error}");
            ApplyFailureState();
            yield break;
        }
        ...
        {
            Debug.LogWarning($"TORTimeAPI could not parse time: {response.current_weather.time}");
            ApplyFailureState();
            yield break;
        }
